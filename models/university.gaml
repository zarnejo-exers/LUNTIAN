...
//	 */
//	 
//	action updateMyNurseries{
//		uni_nurseries <- (plot where each.is_nursery);
//	}
//	 
...
//		do assignNurseries;
//		if(length(uni_nurseries) >= nursery_count){	//start ITP if there's enough nurseries
//			write "Starting ITP: "+length(uni_nurseries)+" with: "+nursery_count;
//			do assignLaborerToNursery(uni_nurseries);
...
//	}
//	 
//	reflex checkIfMustHarvest{
//    	bool to_pause <- true;
//    	loop i over: current_investors{
...
//			do updateTrees;
//		}
//		available_seeds <- (uni_nurseries accumulate each.plot_trees) where (each.age < 4);	//can transplant seed from nursery to ITP until before age 4
...
//			itp_laborers <- [];
//			ask uni_nurseries where (length(each.my_laborers) >1){	//get laborers from nursery with more than one laborer
...
//		loop p over: self.uni_nurseries{
