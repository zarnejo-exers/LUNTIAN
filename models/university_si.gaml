...
global{
	...
	*/
	...
	//(2" x 4" x 8")
	float exotic_price_per_bdft <- (45.06)*(2) update: exotic_price_per_bdft;	//https://forestry.denr.gov.ph/pdf/ds/prices-lumber.pdf 45.06
	float native_price_per_bdft <- (49.35)*(2) update: native_price_per_bdft;	//https://forestry.denr.gov.ph/pdf/ds/prices-lumber.pdf 49.35 
...
//			investor_total_profit, total_investment_cost, total_investments, inv_harvested_trees] rewrite: false to: "../results/0-4*2_percent_earning.csv" format:"csv" header: true;		
//	}
}
...
//	}
	 ...
	 */
	 float computeInvestmentCost(plot investable_plot){
	 	...
	 	float cost_to_support_investment <- (((MAINTENANCE_COST_PER_HA + MAINTENANCE_MATCOST_PER_HA)*(investment_rotation_years))+INIT_ESTABLISHMENT_INVESTOR);
	 	
	 	return cost_to_support_investment;
	 }
			
	float projectProfit(investor inv, plot investable_plot, float i_share){
...
//		//-- harvest process, harvest each of the trees in n parallel steps, where n = # of harvesters hired --
	...
	int harvestITP(investor inv, plot plot_to_harvest, int type_to_harvest, float i_share){	//will harvest only when I already have at least half of wanted nursery
		...
		list<trees> native_trees <- [];
		list<trees> exotic_trees <- [];
    	...
    	if(length(trees_to_harvest) > 0){
	    	exotic_trees <- (trees_to_harvest where (each.type = EXOTIC and !each.is_marked));
	    	if(exotic_trees != []){
	    		do harvestEarning(inv, timberHarvesting(plot_to_harvest, exotic_trees), EXOTIC, i_share);	
	    	}
	    	if(type_to_harvest = BOTH){
	    		native_trees <- (trees_to_harvest where (each.type = NATIVE and !each.is_marked));
		    	if(native_trees != []){
    	...
    	if(inv != nil){
    		inv.total_tree_harvested <- inv.total_tree_harvested + (length(native_trees)+length(exotic_trees));
    	}
